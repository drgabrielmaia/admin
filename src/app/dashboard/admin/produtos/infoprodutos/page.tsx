...
export default function InfoprodutosPage() {
  const { user } = useAuth()
  const [produtos, setProdutos] = useState<InfoprodutoProduto[]>([])
  const [vendas, setVendas] = useState<VendasInfoprodutos>({
    total_vendas: 0,
    faturamento_total: 0,
    lucro_total: 0,
    custo_total: 0
  ...
  const [editingProduct, setEditingProduct] = useState<InfoprodutoProduto | null>(null)
  ...
  const loadData = async () => {
    try {
      ...
      // Carregar produtos infoprodutos
  ...
  const calcularVendas = async () => {
    try {
      ...
      // Buscar todas as vendas APROVADAS de infoprodutos
      const { data: vendasData } = await supabase
        ...
        .eq('produtos.tipo', 'infoproduto')

      // Buscar movimentações manuais do Infoprodutos
      const { data: movimentacoesData } = await supabase
        .from('movimentacoes_financeiras')
        .select('*')
        .eq('negocio', 'infoproduto')
        .eq('status', 'realizado')
  ...
  const handleEdit = (produto: InfoprodutoProduto) => {
  ...
  if (user?.funcao !== 'admin') {
    return (
      <DashboardLayout title="Infoprodutos">
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-2xl font-bold text-white">Infoprodutos Digitais</h2>
          </div>
          <div className="flex space-x-3">
            <Button 
              onClick={() => window.location.href = '/dashboard/admin/produtos/infoprodutos/bpo'}
              variant="outline"
          ...
          <Card className="bg-slate-900 border-slate-800">
            <CardHeader>
              <CardTitle className="flex items-center space-x-2">
                ...
                <span>{editingProduct ? 'Editar Infoproduto' : 'Novo Infoproduto Digital'}</span>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Ex: Curso Completo High-Ticket"
                ...
                <div className="md:col-span-2 space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Descrição detalhada do infoproduto..."
                    className="bg-slate-800 border-slate-700"
                    rows={3}
                  />
                </div>
                ...
                <div className="md:col-span-2 flex gap-2">
                  <Button type="submit" className="bg-blue-600 hover:bg-blue-700">
                    {editingProduct ? 'Atualizar' : 'Criar Infoproduto'}
                  </Button>
                  <Button 
                    type="button" 
                    variant="outline" 
        ...
        <Card className="bg-slate-900 border-slate-800">
          <CardHeader>
            <CardTitle>Infoprodutos Cadastrados</CardTitle>
          </CardHeader>
          <CardContent>
            {produtos.length > 0 ? (
