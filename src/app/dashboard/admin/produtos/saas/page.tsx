...
export default function SaasPage() {
  const { user } = useAuth()
  const [produtos, setProdutos] = useState<SaasProduto[]>([])
  const [vendas, setVendas] = useState<VendasSaas>({
    total_vendas: 0,
    faturamento_total: 0,
    lucro_total: 0,
    custo_total: 0
  ...
  const calcularVendas = async () => {
    try {
      ...
      // Buscar todas as vendas APROVADAS de produtos SaaS
      const { data: vendasData } = await supabase
        ...
        .eq('produtos.tipo', 'saas')

      // Buscar movimentações manuais do SaaS
  ...
  const handleEdit = (produto: SaasProduto) => {
  ...
  if (user?.funcao !== 'admin') {
    return (
      <DashboardLayout title="Produtos SaaS">
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-2xl font-bold text-white">Produtos SaaS</h2>
          </div>
          <div className="flex space-x-3">
            <Button 
              onClick={() => window.location.href = '/dashboard/admin/produtos/saas/bpo'}
              variant="outline"
          ...
          <Card className="bg-slate-900 border-slate-800">
            <CardHeader>
              <CardTitle className="flex items-center space-x-2">
                ...
                <span>{editingProduct ? 'Editar Produto' : 'Novo Produto SaaS'}</span>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Ex: Platform Premium High-Ticket"
                ...
                <div className="md:col-span-2 space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Descrição detalhada do produto SaaS..."
                    className="bg-slate-800 border-slate-700"
                    rows={3}
                  />
                </div>
        ...
        <Card className="bg-slate-900 border-slate-800">
          <CardHeader>
            <CardTitle>Produtos SaaS Cadastrados</CardTitle>
          </CardHeader>
          <CardContent>
            {produtos.length > 0 ? (
