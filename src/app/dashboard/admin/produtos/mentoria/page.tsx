...
export default function MentoriaPage() {
  const { user } = useAuth()
  const [produtos, setProdutos] = useState<MentoriaProduto[]>([])
  const [vendas, setVendas] = useState<VendasMentoria>({
    total_vendas: 0,
    faturamento_total: 0,
    lucro_total: 0,
    custo_total: 0
  ...
  const [editingProduct, setEditingProduct] = useState<MentoriaProduto | null>(null)
  ...
  const calcularVendas = async () => {
    try {
      ...
      // Buscar todas as vendas APROVADAS de produtos de mentoria
      const { data: vendasData } = await supabase
        ...
        .eq('produtos.tipo', 'mentoria')

      // Buscar movimentações manuais de mentoria
  ...
  const handleEdit = (produto: MentoriaProduto) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (error) {
        console.error('Erro de permissão ou constraint:', error)
        alert('Não foi possível excluir o produto. Pode estar sendo usado em outras operações.')
        return
      }
  ...
  if (user?.funcao !== 'admin') {
    return (
      <DashboardLayout title="Mentoria">
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-2xl font-bold text-white">Produtos de Mentoria</h2>
          </div>
          <div className="flex space-x-3">
            <Button 
              onClick={() => window.location.href = '/dashboard/admin/produtos/mentoria/bpo'}
              variant="outline"
          ...
          <Card className="bg-slate-900 border-slate-800">
            <CardHeader>
              <CardTitle className="flex items-center space-x-2">
                ...
                <span>{editingProduct ? 'Editar Produto' : 'Novo Produto de Mentoria'}</span>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Ex: Mentoria VIP 1:1"
