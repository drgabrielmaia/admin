...
export default function ClinicaPage() {
  const { user } = useAuth()
  const [produtos, setProdutos] = useState<ClinicaProduto[]>([])
  const [vendas, setVendas] = useState<VendasClinica>({
    total_vendas: 0,
    faturamento_total: 0,
    lucro_total: 0,
    custo_total: 0
  ...
  const [editingProduct, setEditingProduct] = useState<ClinicaProduto | null>(null)
  ...
  const calcularVendas = async () => {
    try {
      ...
      const { data: vendasData } = await supabase
        ...
        .eq('produtos.tipo', 'clinica')

      // Buscar movimentações manuais da clínica
  ...
  const handleEdit = (produto: ClinicaProduto) => {
  ...
  if (user?.funcao !== 'admin') {
    return (
      <DashboardLayout title="Produtos Clínica">
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-2xl font-bold text-white">Produtos Clínica</h2>
          </div>
          <div className="flex space-x-3">
            <Button 
              onClick={() => window.location.href = '/dashboard/admin/produtos/clinica/bpo'}
              variant="outline"
          ...
          <Card className="bg-slate-900 border-slate-800">
            <CardHeader>
              <CardTitle className="flex items-center space-x-2">
                ...
                <span>{editingProduct ? 'Editar Produto' : 'Novo Produto Clínica'}</span>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Ex: Consulta Premium"
                ...
                <div className="md:col-span-2 space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Descrição detalhada do produto da clínica..."
                    className="bg-slate-800 border-slate-700"
                    rows={3}
                  />
                </div>
        ...
        <Card className="bg-slate-900 border-slate-800">
          <CardHeader>
            <CardTitle>Produtos Clínica Cadastrados</CardTitle>
          </CardHeader>
          <CardContent>
            {produtos.length > 0 ? (
                  ...
                  <div key={produto.id} className="flex items-center justify-between p-4 rounded-lg bg-slate-800">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3">
                        ...
                        <Badge className="bg-rose-900 text-rose-300">
